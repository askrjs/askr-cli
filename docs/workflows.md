...
The config must provide the canonical `siteUrl`; the CLI aligns `sitemap.xml`
with rendered canonical links and writes `.askr/ssg-output.json` alongside the
route HTML and build metadata. Configure per-route
