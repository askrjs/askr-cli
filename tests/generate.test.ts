...
const document = {
  ...
  components: { securitySchemes: { bearer: { type: "http", scheme: "bearer" } }, schemas: { User: { type: "object", required: ["id"], properties: { id: { type: "string" }, parent: { $ref: "#/components/schemas/User" } } } } },
  paths: {
    "/users/{id}": {
      get: {
        operationId: "getUser", security: [{ bearer: [] }],
        parameters: [
          { in: "path", name: "id", required: true, schema: { type: "string" } },
          { in: "query", name: "include", style: "form", explode: true, schema: { type: "string" } },
        ],
        responses: {
          "200": { description: "ok", content: { "application/json": { schema: { $ref: "#/components/schemas/User" } } } },
          "404": { description: "missing", content: { "application/problem+json": { schema: { type: "object", properties: { title: { type: "string" } } } } } },
        },
      },
    },
  },
...
describe("askr generate", () => {
  ...
  it("should require operation ids given an operation when rendering", () => {
    const invalid: any = structuredClone(document); delete invalid.paths["/users/{id}"].get.operationId; expect(() => generateFiles(invalid)).toThrow(/operationId is required/);
  });
  it("should refuse unowned output given a nonempty directory when writing", async () => {
    const output = await mkdtemp(join(tmpdir(), "askr-unowned-")); await writeFile(join(output, "mine.txt"), "keep"); await expect(writeGenerated(output, generateFiles(document), false)).rejects.toThrow(/Refusing to overwrite/); expect(await readFile(join(output, "mine.txt"), "utf8")).toBe("keep");
  });
  it("should detect stale and extra files without writes given check mode when checking", async () => {
    const root = await mkdtemp(join(tmpdir(), "askr-check-")); const output = join(root, "client"); const files = generateFiles(document); await writeGenerated(output, files, false); await writeFile(join(output, "extra.ts"), "x"); await expect(writeGenerated(output, files, true)).rejects.toThrow(/ownership differs/); expect((await readdir(output)).sort()).toContain("extra.ts");
  });
