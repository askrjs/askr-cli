...
`src/routes.tsx` exports one route registry shared by the browser and server. The server entry composes `createServerApp()` with `createAskrPageHandler()`, which returns an Askr HTML fragment. The Vite server integration transforms `index.html` and inserts that fragment at the single `<!--askr-app-->` marker.
...
```text
index.html            document source and sole askr-app marker
