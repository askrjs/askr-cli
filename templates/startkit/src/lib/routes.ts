...
export const protectedRoutes = [dashboardRoute, accountsRoute, settingsRoute] as const;
export const workspaceRoutes = [...protectedRoutes] as const;
export const routeCatalog = [landingRoute, loginRoute, ...protectedRoutes] as const;
...
export function normalizeProtectedRouteTarget(target: string | null | undefined): string {
  if (!target) {
    return defaultProtectedRoute.href;
  }

  const parsed = parseRouteTarget(target);
  if (!parsed || parsed.origin !== appOrigin || !isProtectedRoute(parsed.pathname)) {
    return defaultProtectedRoute.href;
  }
