  ...
  return (
    <Card class="counter" variant="raised">
      <CardHeader class="counter__header">
        ...
        <CardDescription>
          <code>state()</code> drives the count. <code>derive()</code> keeps parity current.
        </CardDescription>
      </CardHeader>
      <CardContent class="counter__body">
        ...
        <Flex wrap="wrap" class="counter-controls">
          <Button variant="secondary" onPress={() => count.set((value) => Math.max(0, value - 1))}>
            -
          </Button>
          <Button variant="primary" onPress={() => count.set((value) => value + 1)}>
            +
          </Button>
        </Flex>
      </CardContent>
